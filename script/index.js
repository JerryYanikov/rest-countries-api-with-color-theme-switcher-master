    ...
    .then(response => response.json()).then(
        function retrive(data) {
            for (let i = 0; i < data.length; i++) {
                ...
                countryCard.innerHTML = `<img class="country-flag" id="countryFlag" src=${data[i].flags[0]}></img><div class="country-details"><h2 id="countryName">${data[i].name}</h2><div class="country-data-line"><div class="country-data-description">Population:</div><div id="poplation">${(data[i].population).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</div></div><div class="country-data-line"><div class="country-data-description">Region:</div><div id="region">${data[i].region}</div></div><div class="country-data-line"><div class="country-data-description">Capital:</div><div id="capital">${data[i].capital}</div></div></div>`;
